...
estimate = 0; % 0 = estimate all parameters, 1:6 = estimate individual params, 7 estimate M
p0 = log(2); % initial value for the parameter that you want to estimate
...
%  x0=[  1.8540   -4.5461   -3.0421    0.0007    3.1017   -0.7082]; % .0953
 x0 = [  1.8776   -4.5708   -3.0938    0.0007    3.0900   -0.7021]; % .0821
 %x0 = [  1.8776   -4.5708   -3.0938    0.0007    log(.7)   -0.7021]; % .0821
...
if sw==1  % fit data
    ...
    if estimate == 0
        ...
        if estimate < 7
            [p sse] = fminsearch(@(p) fitMDFTs([x0(1:estimate-1), p, x0(estimate + 1: 6)],D,FD,MM,C3,Ns), p0,options)
            
            x = [x0(1:estimate-1), p, x0(estimate + 1: 6)];
        else
            M0 = ones(3,2,10);
            [M sse] = fminsearch(@(M) fitMDFTs(x0,D,FD,exp(M),C3,Ns), log(M0),options);
            x = x0;
            MM = exp(M);
        end
    end
