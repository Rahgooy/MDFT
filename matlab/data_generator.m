        ...
        for ncomb=combs % Number of combinations of options that sampled
            ...
            for j = 1:nproblem
                ...
                for i = 1:ncomb
                    ...
                    f = sprintf('%s(G,C3,M,w,theta,sig2,Ns,pref_based)', simName);
                    [p3, T] = eval(f); %simMDF_mex(G,C3,M,w,theta,sig2,Ns);
                    times(i) = T;
                    D(i, :) = p3;
                    idx(i, :) = id;
                end
                dataset{j}.pref_based = pref_based;
                dataset{j}.M = MM;
            ...
            o = sprintf('../data/%s/set_nopts%d_ncomb%d_nproblem%d_no%d.mat', name, nopts, ncomb, nproblem, na);
            save(o, 'dataset')
        end
    end
end
